...
for _ in $(seq 1 60); do
  ...
  [[ "$code" == "200" ]] && fail "Job completed before we could cancel it — workload may be too small for this test"
  sleep 1
done
...
echo "🛑 Cancel..."
curl -s -X POST \
  "${TORCH_BASE_URL}/fhir/Task/${EXPORT_ID}/\$cancel" \
  -H "Accept: application/fhir+json" \
  -o /dev/null -w '%{http_code}' | grep -q 200 || fail "Cancel failed"

echo "⏳ Waiting for 410..."
for _ in $(seq 1 60); do
  code="$(status_code "$EXPORT_ID")"
  [[ "$code" == "410" ]] && break
  [[ "$code" == "200" ]] && fail "Completed instead of cancelled"
  sleep 1
done

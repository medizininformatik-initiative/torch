...
3. **Load References**: Torch checks if it already loaded the referenced resources.
...
4. **Assign References**: Torch assigns the loaded resources to the ResourceGroup.
    - Performs a must-have check on the loaded resources.
    - For Patient resources, it also checks if the resources are allowed by the patient's consent.

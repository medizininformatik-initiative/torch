...
public class ConsentAdjuster {
     ...
     */
    public Mono<Map<String, List<ConsentProvisions>>> fetchEncounterAndAdjustByEncounter(PatientBatch batch, List<ConsentProvisions> provisions) {
    ...
    public Map<String, List<ConsentProvisions>> adjustProvisionsByEncounters(
            List<ConsentProvisions> provisions,
            Map<String, Collection<Encounter>> encountersByPatient
    ) {
        return provisions.stream()
                .collect(Collectors.groupingBy(ConsentProvisions::patientId)) // group by patient
                .entrySet().stream()

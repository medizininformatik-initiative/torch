...
public class ConsentCalculator {
     ...
     */
    Map<String, NonContinuousPeriod> calculateConsentPeriodsByCode(
            List<ConsentProvisions> consentProvisions,
            String consentKey
    ) {
        // Get relevant codes for this consent key
        Set<String> codes = mapper.getRelevantCodes(consentKey);
        ...
        return result.keySet().equals(codes) ? result : Map.of();
    }
     ...
     * <ul>
     *     <li>Calculates allowed periods per code using {@link #calculateConsentPeriodsByCode(List, String)}</li>
                ...
                .flatMap(entry -> {
                    ...
                    try {
                        NonContinuousPeriod finalConsent =
                                intersectConsent(calculateConsentPeriodsByCode(provisions, consentKey));

...
public class ConsentCalculator {
     ...
     * <p>
     * Resources are processed in ascending {@link ConsentProvisions#dateTime()} order (ties broken by
     * {@link ConsentProvisions#id()} for determinism), one code at a time, folding each resource's
     ...
     * and a resource denying any of a code's configured retro modifiers discards that code's entire
     * history so far rather than merely subtracting from it — a matching retro modifier permit does not,
     * since a revocation demands a conservative reset but a grant should never leave a patient worse off.
     *
     ...
     * the plain permit period on their own. If the resource carries a <b>deny</b> of any of {@code entry}'s
     * retro modifier codes — regardless of overlap — the resulting contribution <b>replaces</b>
     * {@code running} outright rather than merging into it, discarding that code's history so far; a retro
     * modifier permit with no matching {@code code} permit in the same resource has no such effect, since it
     * has nothing to extend and nothing to revoke. Finally, denies of {@code code} itself (from any
    ...
    private NonContinuousPeriod applyResource(
        ...
        boolean retroTouched = !retroCodes.isEmpty() && provisions.stream()
                .anyMatch(p -> !p.permit() && retroCodes.contains(p.code()));

        NonContinuousPeriod combined = retroTouched ? contribution : running.merge(contribution);

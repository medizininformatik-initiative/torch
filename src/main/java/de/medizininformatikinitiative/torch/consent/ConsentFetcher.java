...
public class ConsentFetcher {
     ...
     */
    public Mono<List<ConsentProvisions>> fetchConsentInfo(String key, PatientBatch batch) {
        ...
        return dataStore.search(getConsentQuery(batch), Consent.class)
                ...
                .flatMap(consents -> {
                    ...
                    List<ConsentProvisions> allProvisions = consents.values().stream()
                            ...
                            .toList();
                    return Mono.just(allProvisions);
                });
    }
}

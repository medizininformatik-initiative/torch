...
public record JobDiagnosticSummary(@JsonProperty("Num-Cohort-Patients") int numCohortPatients,
     ...
     */
    public JobDiagnosticSummary initFromBatches(List<BatchDiagnostics> batchDiagnostics) {
    ...
    /**
     * Computes the average and median elapsed nanoseconds for each {@link PipelineStage} across all batch diagnostics.
     *
     * @param batchDiagnostics  the batch diagnostics of each batch of the job
     * @return                  the accumulated durations per {@link PipelineStage}
     */
    private Map<PipelineStage, DurationSummary> computeTimingSummary(List<BatchDiagnostics> batchDiagnostics) {
        ...
        merged.forEach((stage, nanos) -> {
            ...
            durations.put(stage, new DurationSummary(median, average));
        });
     ...
     */
    private int sumCohortPatients(List<BatchDiagnostics> diagnostics) {
     ...
     */
    private int sumFinalPatients(List<BatchDiagnostics> diagnostics) {
     ...
     */
    private Map<String, GroupSummary> computeResourceSummaries(List<BatchDiagnostics> diagnostics) {
        ...
        diagnostics.forEach(batch -> batch.batchExclusions().getResourceExclusions().forEach(event -> {
            ...
            GroupSummary newGroupSummary = switch(event.reason()) {
                ...
                case REFERENCE_INVALID -> exclusionsPerGroup.get(event.groupId()).incrementInvalidRef();
            };
     ...
     */
    private Map<PatientExclusionStage, Integer> computePatientSummaries(List<BatchDiagnostics> diagnostics) {
     ...
     * @param refInvalidRefExclusions   the sum of invalid-reference exclusions
     */
    private record GroupSummary(@JsonProperty("Must-Have") Map<String, Integer> mustHaveExclusions,
                                ...
                                @JsonProperty("Reference-Invalid") int refInvalidRefExclusions) {
        ...
        public GroupSummary incrementConsent() {
            return new GroupSummary(mustHaveExclusions, consentExclusions+1, refNotFoundExclusions, resOutsideBatchExclusions, refInvalidRefExclusions);
        }
        public GroupSummary incrementRefNotFound() {
            return new GroupSummary(mustHaveExclusions, consentExclusions, refNotFoundExclusions+1, resOutsideBatchExclusions, refInvalidRefExclusions);
        }
        public GroupSummary incrementResOutsideBatch() {
            return new GroupSummary(mustHaveExclusions, consentExclusions, refNotFoundExclusions, resOutsideBatchExclusions+1, refInvalidRefExclusions);
        }
        public GroupSummary incrementInvalidRef() {
            return new GroupSummary(mustHaveExclusions, consentExclusions, refNotFoundExclusions, resOutsideBatchExclusions, refInvalidRefExclusions+1);
        }
    }
     ...
     *
     * @param medianNanos   an average amount of nanoseconds across multiple batches
     * @param averageNanos  a median amount of nanoseconds across multiple batches
     */
    private record DurationSummary(@JsonProperty Long medianNanos, @JsonProperty Long averageNanos){}

 ...
 */
public record CoreResult(UUID jobId, List<Issue> issues, WorkUnitStatus status, Optional<BatchDiagnostics> diagnostics) {
    public CoreResult(UUID jobId, List<Issue> issues, WorkUnitStatus status) {
        this(jobId, issues, status, Optional.empty());
    }
}

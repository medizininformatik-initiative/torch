    ...
    public Mono<Void> processCoreAttributeGroup(AnnotatedAttributeGroup group,
        ...
        return groupQueries(group)
                ...
                .flatMap(resource -> {
                    ...
                    boolean valid = eval.applicable() && eval.fulfilled();

                    if (valid) {
                        atLeastOneResource.set(true);
                    } else if (eval.applicable()) {
                        eval.firstViolated().ifPresent(attr ->
                                acc.incResourcesExcluded(CriterionKeys.mustHaveAttribute(group, attr), 1)
                        );
                    }
        ...
        return groupQueries(group)
                ...
                .doOnNext(tuple -> {
                    ...
                    if (eval.fulfilled()) {
                        ...
                        // resource-level attribution: first failing must-have attribute
                        eval.firstViolated().ifPresent(attr ->
                                acc.incResourcesExcluded(CriterionKeys.mustHaveAttribute(group, attr), 1)
                        );
                    }
                ...
                .doFinally(signal -> {
                    if (signal != SignalType.ON_COMPLETE) {
                        logger.debug("Group {} terminated with signal {}", group.id(), signal);
                    }

                    // patient-level attribution when we shrink the global safeSet
                    if (group.hasMustHave()) {

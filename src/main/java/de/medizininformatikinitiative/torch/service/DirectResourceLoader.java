...
public class DirectResourceLoader {
    ...
    private Mono<DomainResource> applyConsent(DomainResource resource,
        ...
        try {
            ...
            patientBatchWithConsent.batchExclusions().addConsentExclusion(group.id(), resource.getId(), patientID);
        ...
        return groupQueries(group)
                ...
                .flatMap(resource -> {
                    ...
                    if (valid) {
                        ...
                        // TODO use getRelativeURL
                        batchExclusions.addMustHaveExclusionCore(group.groupReference(), resource.getId(), v.firstViolated().attributeRef());
                    }
                ...
                .then(Mono.defer(() -> {
                    ...
                    return Mono.error(new MustHaveViolatedException.GroupViolated(
                            "MustHave requirement violated for group: " + group.id(), Set.of(group.id())));
                }));
    }
        ...
        return resourceFlux
                ...
                .doOnNext(tuple -> {
                    ...
                    if (eval instanceof MustHaveEvaluation.Fulfilled) {
                        ...
                        bundle.put(tuple.resource, group.id(), false);
                        batch.batchExclusions().addMustHaveExclusion(group.groupReference(), tuple.resource.getIdPart(), v.firstViolated().attributeRef(), tuple.patientId);
                    }
                })
                .doFinally(signal -> {
                    if (group.hasMustHave()) {

...
import static de.medizininformatikinitiative.torch.diagnostics.exclusions.PatientExclusionStage.CASCADING_DELETE;
        ...
        Set<String> survivingPatientIds = batch.bundles().entrySet().stream()
                .filter(entry -> {
                    try {
                        ...
                        batch.diagnostics().batchExclusions().addPatientExclusion(CASCADING_DELETE, entry.getKey());
                        return false;
                    }
                })

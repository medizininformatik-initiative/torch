...
public class ReferenceResolver {
     ...
     * @param batchExclusions diagnostics accumulator to note newly excluded resources (gets mutated)
    ...
    private Set<ExtractionId> setUnloadedAsInvalid(List<Resource> fetchedResources,
        ...
        refToPatHelper.forEach((wrapper, patID) -> wrapper.references().forEach(unknownRef -> {
            if (notLoaded.contains(unknownRef)) {
                ...
                batch.batchExclusions().addReferenceNotFoundExclusion(resourceGroup.groupId(), resourceGroup.resourceId().toRelativeUrl(), patID);
    ...
    private Set<ExtractionId> setUnloadedAsInvalidCore(List<Resource> fetchedResources,
        ...
        notLoaded.forEach(missingRef -> {
            ...
            batchExclusions.addReferenceNotFoundExclusionCore(resourceGroup.groupId(), resourceGroup.resourceId().toRelativeUrl());
        });
        ...
        try {
            ...
            if (group != null) {
                patId.ifPresentOrElse(
                        pat -> batchExclusions.addMustHaveExclusion(resourceGroup.groupId(), resourceGroup.resourceId().toRelativeUrl(), e.getAttributeRef(), pat),
                        () -> batchExclusions.addMustHaveExclusionCore(resourceGroup.groupId(), resourceGroup.resourceId().toRelativeUrl(), e.getAttributeRef()));
            }
            return Map.entry(resourceGroup, Collections.emptyList());
        }
    }

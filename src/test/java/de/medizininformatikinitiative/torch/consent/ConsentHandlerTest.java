...
public class ConsentHandlerTest {
    ...
    @Test
    void registersPatientBeforeFailingOnNoConsentPeriods() {
        var codes = CODES;
        ...
        StepVerifier.create(consentHandler.fetchAndBuildConsentInfo(codes, batch))
                .expectErrorSatisfies(error -> assertThat(error).isInstanceOf(ConsentViolatedException.class))
                .verify();

        // registered eagerly before the fetch, so the patient still shows up in the consent-trail folder
        // even though this batch never reaches PatientBatchWithConsent — see ExtractDataService.processBatch,
        // which persists batch.diagnostics() unchanged when the whole batch is skipped for lack of consent
        assertThat(batch.diagnostics().consentDiagnostics().getPatientIds()).containsExactly(PATIENT_ID);
    }

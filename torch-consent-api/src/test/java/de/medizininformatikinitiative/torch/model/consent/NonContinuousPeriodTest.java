...
class NonContinuousPeriodTest {
    ...
    class Meta {
        ...
        @Test
        void ofSinglePeriod_wrapsItInASingleElementList() {
            var period = p("2024-01-01", "2024-01-10");

            var ncp = NonContinuousPeriod.of(period);

            assertThat(ncp.periods()).containsExactly(period);
        }

        @Test
        void containsDate_trueOnlyWhenDateFallsWithinAPeriod() {
            var ncp = new NonContinuousPeriod(List.of(p("2024-01-01", "2024-01-10")));

            assertThat(ncp.containsDate(LocalDate.parse("2024-01-05"))).isTrue();
            assertThat(ncp.containsDate(LocalDate.parse("2024-02-01"))).isFalse();
        }

...
public class CrtdlConsentValidator {
    ...
    /**
     * Extracts all consent codes from the inclusion criteria of a CRTDL,
     * while validating both inclusion and exclusion rules.
     *
     * @param crtdl the CRTDL containing the cohort definition
     ...
     */
    public Optional<Set<TermCode>> extractConsentCodes(ConsentContext crtdl) throws ConsentFormatException {
     ...
     */
    private Set<TermCode> collectInclusionTermCodes(JsonNode cohortDefinition) {
        ...
        for (JsonNode group : inclusionCriteria) {
            ...
            processInclusionGroup(group).forEach(allCodes::add);
        }
        return allCodes;
    }

    /**
     * Processes a single inclusion group, collecting term codes from all Einwilligung criteria.
     */
    private Stream<TermCode> processInclusionGroup(JsonNode group) {
